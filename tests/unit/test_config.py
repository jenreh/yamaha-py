...
class TestLoadProfile:
    def test_loads_default(self, sample_config) -> None:
        cfg = load_profile()
        assert cfg.name == "test"
        assert cfg.host == "192.168.1.100"
        assert cfg.protocol == "http_xml"

    def test_loads_named(self, sample_config) -> None:
        cfg = load_profile("test")
        assert cfg.host == "192.168.1.100"

    def test_missing_config_file(self, tmp_config) -> None:
        with pytest.raises(ConfigError, match="No config found"):
            load_profile()

    def test_missing_profile(self, sample_config) -> None:
        with pytest.raises(ConfigError, match="not found"):
            load_profile("nonexistent")

    def test_no_default_profile(self, tmp_config) -> None:
        tmp_config.write_text(yaml.dump({"profiles": {"test": {"host": "1.2.3.4"}}}))
        with pytest.raises(ConfigError, match="No default_profile"):
            load_profile()

    def test_malformed_config(self, tmp_config) -> None:
...
class TestSaveProfile:
    def test_creates_new_profile(self, tmp_config) -> None:
    ...
    def test_updates_existing(self, sample_config) -> None:
    ...
    def test_preserves_other_profiles(self, sample_config) -> None:
    ...
    def test_creates_config_dir(self, tmp_config) -> None:
...
class TestListProfiles:
    def test_returns_profile_names(self, sample_config) -> None:
        profiles = list_profiles()
        assert "test" in profiles

    def test_empty_when_no_config(self, tmp_config) -> None:
        profiles = list_profiles()
        assert profiles == {}


class TestSetDefaultProfile:
    def test_sets_default(self, sample_config) -> None:
        cfg = ReceiverConfig(name="other", host="10.0.0.1")
        save_profile(cfg)
        set_default_profile("other")
        assert get_default_profile() == "other"

    def test_missing_profile_raises(self, sample_config) -> None:
        with pytest.raises(ConfigError, match="not found"):
