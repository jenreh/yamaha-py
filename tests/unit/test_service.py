...
class TestProtocolSelection:
    def test_selects_http_xml_adapter(self):
    ...
    def test_selects_ynca_adapter(self):
...
class TestVolumeValidation:
    ...
    def test_rounds_to_half_step(self):
        svc = _make_service()
...
class TestRetryLogic:
    ...
    def test_no_retry_on_unexpected_response(self, mocker):
...
class TestZoneOverride:
    def test_zone_override_applied(self, tmp_path):
        ...
        config_file.write_text(yaml.dump({
            ...
            "profiles": {"test": {"host": "192.168.1.1", "protocol": "http_xml", "zone": "main"}},
        }))
...
class TestSceneValidation:
    def test_invalid_scene_raises(self, mocker):
        svc = _make_service()
        with pytest.raises(ValueError, match="Scene must be 1"):
            svc.load_scene(5)

    def test_valid_scenes(self, mocker):
        svc = _make_service()
