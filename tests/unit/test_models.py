...
class TestDiscoveryCandidate:
    def test_confidence_high(self):
        c = DiscoveryCandidate(host="192.168.1.1", ynca_available=True, http_available=True)
        assert c.confidence == "high"

    def test_confidence_medium_ynca(self):
        c = DiscoveryCandidate(host="192.168.1.1", ynca_available=True)
        assert c.confidence == "medium"

    def test_confidence_medium_http(self):
    ...
    def test_protocol_prefers_ynca(self):
        c = DiscoveryCandidate(host="192.168.1.1", ynca_available=True, http_available=True)
        assert c.protocol == "ynca"

    def test_protocol_fallback_http(self):
        c = DiscoveryCandidate(host="192.168.1.1", http_available=True)
