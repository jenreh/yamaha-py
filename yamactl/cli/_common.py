...
ProfileOpt = Annotated[Optional[str], typer.Option("--profile", "-p", help="Config profile name.")]
ZoneOpt = Annotated[Optional[str], typer.Option("--zone", "-z", help="Zone: main or zone2.")]
...
def make_service(profile: str | None, zone: str | None) -> ReceiverService:
    ...
    if zone is not None:
        if zone not in ("main", "zone2"):
            typer.echo(f"Error: --zone must be 'main' or 'zone2', got '{zone}'", err=True)
            raise typer.Exit(2)
